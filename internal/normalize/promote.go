...
func PromotePrompt(transcript, source string, vocabulary, existingIDs, projects []string) string {
	...
	b.WriteString("- non-obvious facts about the environment, tooling or a project\n")
	b.WriteString("- corrections the user gave, especially repeated ones\n\n")
	b.WriteString("Never extract: anything derivable from reading the code, transient state\n")
	b.WriteString("(what was being debugged, branch names, what ran today), restatements of\n")
	b.WriteString("official documentation, or the assistant's own explanations.\n\n")
